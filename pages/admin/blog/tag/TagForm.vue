<template>
    ...
    <div v-if="model" class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <ToolBar>
            <!--begin::Buttons-->
            <router-link :to="{name: 'admin.blog.tags'}" class="btn btn-sm btn-flex btn-light fw-bolder me-2">{{ $t('back') }}</router-link>
...
export default {
    ...
    setup(){
        ...
        const create = () => {
            const data = getData()
            ApiService.post('/blog/tags', data)
                .then((response) => {
                    if(response.data.success){
                        initModel()
                        toastr.success(t('messages.tags.created'))
                    }else{
        ...
        const update = () => {
            const data = getData()
            ApiService.put('/blog/tags/' + id.value, data)
                .then((response) => {
                    if(response.data.success){
                        initModel(response.data.data)
                        toastr.success(t('messages.tags.updated'))
                    }else{
                        toastr.error(t('error'))
                    }
                })
        ...
        const loadModel = (value) => {
            if(!value){ return; }
            ApiService.get('/blog/tags/' + value)
                .then((response) => {
                    if(response.data.success){
                        initModel(response.data.data)
                    }
