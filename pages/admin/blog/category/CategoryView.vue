<template>
    ...
    <div v-if="model" class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <ToolBar>
            <!--begin::Buttons-->
            <router-link :to="{name: 'admin.blog.categories.edit'}" class="btn btn-sm btn-flex btn-primary fw-bolder me-2">{{ $t('edit') }}</router-link>
            <!--end::Buttons-->
        </ToolBar>
        <!--end::Toolbar-->
                ...
                <div class="card mb-6 mb-xl-9">
                    <div class="card-body pt-9 pb-0">
                        ...
                        <ul class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bolder">
                            <li class="nav-item">
                                <router-link :to="{name:'admin.blog.categories.view'}" :class="{'active': isActive('admin.categories.view')}" class="nav-link text-active-primary py-5 me-6">{{ $t('versions') }}</router-link>
                            </li>
                        </ul>
                        <!--end::Nav-->
                    </div>
    ...
    setup(){
        ...
        onMounted(() => {
            store.dispatch(SET_BREADCRUMB, [
                ...
                {title: 'categories', route: 'admin.blog.categories'},
                {title: 'Category'},
            ])
        })
        ...
        const loadModel = () => {
            ApiService.get('/blog/categories/' + route.params.id)
                .then(({data}) => {
                    if(data.success) {
