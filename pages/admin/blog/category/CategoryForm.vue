<template>
    ...
    <div v-if="model" class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <ToolBar>
            <!--begin::Buttons-->
            <router-link :to="{name: 'admin.blog.categories'}" class="btn btn-sm btn-flex btn-light fw-bolder me-2">{{ $t('back') }}</router-link>
...
export default {
    ...
    setup(){
        ...
        const create = () => {
            const data = getData()
            ApiService.post('/blog/categories', data)
                .then((response) => {
        ...
        const update = () => {
            const data = getData()
            ApiService.put('/blog/categories/' + id.value, data)
                .then((response) => {
                    if(response.data.success){
        ...
        const loadModel = (value) => {
            if(!value){ return; }
            ApiService.get('/blog/categories/' + value)
                .then((response) => {
                    if(response.data.success){
                        initModel(response.data.data)
                    }
