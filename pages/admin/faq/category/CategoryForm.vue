<template>
    ...
    <div v-if="model" class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <ToolBar>
            <!--begin::Buttons-->
            <router-link :to="{name: 'admin.faq.categories'}" class="btn btn-sm btn-flex btn-light fw-bolder me-2">{{ $t('back') }}</router-link>
                ...
                <div class="card mb-5 mb-xl-10">
                    ...
                    <form @submit.prevent="submit" id="form" method="post" class="form fv-plugins-bootstrap5 fv-plugins-framework" novalidate="novalidate">
                        ...
                        <div class="card-body border-top p-9">
                            ...
                            <div class="row mb-6">
                                <!--begin::Label-->
                                <label class="col-xl-2 col-lg-4 col-form-label fw-bold fs-6">{{ $t('title') }}</label>
                                <!--end::Label-->
                                <!--begin::Col-->
                                <div class="col-xl-10 col-lg-8 fv-row">
...
export default {
    ...
    setup(){
        ...
        const create = () => {
            const data = getData()
            ApiService.post('/faq/categories', data)
                .then((response) => {
                    if(response.data.success){
                        initModel()
                        toastr.success(t('messages.categories.created'))
                    }else{
        ...
        const update = () => {
            const data = getData()
            ApiService.put('/faq/categories/' + id.value, data)
                .then((response) => {
                    if(response.data.success){
                        initModel(response.data.data)
                        toastr.success(t('messages.categories.updated'))
                    }else{
                        toastr.error(t('error'))
                    }
                })
        ...
        const loadModel = (value) => {
            if(!value){ return; }
            ApiService.get('/faq/categories/' + value)
                .then((response) => {
                    if(response.data.success){
                        initModel(response.data.data)
                    }
