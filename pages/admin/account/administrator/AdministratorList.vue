                    ...
                    <div class="card-body pt-0">
                        ...
                        <table class="table align-middle table-row-dashed fs-6 gy-5">
                            ...
                            <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                                ...
                                <th class=" min-w-70px text-end">{{ $t('actions') }}</th>
                            </tr>
                            <!--end::Table row-->
                            </thead>
                            <!--end::Table head-->
                            ...
                            <tr
                                ...
                                <!--begin::Checkbox-->
                                <td>
                                    <div v-if="!isCurrentUser(model)" class="form-check form-check-sm form-check-custom form-check-solid">
                                        <input class="form-check-input" type="checkbox" v-model="selected" :value="model" />
                                    </div>
                                </td>
                                <!--end::Checkbox-->
                                <td>
                                    <router-link :to="{name: 'admin.administrators.view', params: {id: model.id}}" class="d-flex align-items-center">
                                ...
                                <td>
                                    <div class="d-flex justify-content-start flex-shrink-0">
                                        <span
                                            ...
                                            :key="user.id"
                                            class="badge badge-light-warning text-dark me-2 mb-2">
                                            {{ user.username }}
                                        </span>
                                    </div>
                                </td>
                                <td>
                                    <span class="text-dark text-hover-primary fs-6">{{ $filters.formatDateTime(model.created_at) }}</span>
                                </td>
                                ...
                                <td class="text-end">
                                    ...
                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                        ...
                                        <div class="menu-item px-3">
                                            <router-link  v-if="model.id" :to="{name: 'admin.administrators.view', params: {id: model.id}}" class="menu-link px-3">{{ $t('view') }}</router-link>
                                        </div>
                                        <!--end::Menu item-->
                                        <!--begin::Menu item-->
                                        <div class="menu-item px-3">
                                            <router-link  v-if="model.id" :to="{name: 'admin.administrators.edit', params: {id: model.id}}" class="menu-link px-3">{{ $t('edit') }}</router-link>
                                        </div>
                                        <!--end::Menu item-->
                                        <!--begin::Menu item-->
                                        <div v-if="!isCurrentUser(model)" class="menu-item px-3">
                            ...
                            <tr class="odd">
                                <td colspan="5">{{ $t('no_administrator_found') }}</td>
                            </tr>
                            </tbody>
                            <!--end::Table body-->
                        </table>
...
export default {
    ...
    setup(){
        ...
        const loadModels = () => {
            ...
            ApiService.get('/administrators', {params: query}, true)
                .then(({data}) => {
                    if(data.success) {
        ...
        const handleDelete = (model) => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ApiService.delete('/administrators/' + model.id)
                        .then(response => {
        ...
        const handleMassDelete = () => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    ApiService.delete('/administrators', {data: {administrators: data}})
                        .then(response => {
                            if(response.data.success){
        ...
        const isCurrentUser = (value) => {
            return value && currentUser.value && currentUser.value.account && currentUser.value.account.id === value.id
        }
