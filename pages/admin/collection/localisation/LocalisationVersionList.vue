        ...
        <div class="card-header border-0 pt-6">
            ...
            <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                ...
                <div class="w-100 mw-250px">
                    <InputAttachedSelect v-model="attached" />
                </div>
                <!--end::Attached-->
                <!--begin::Titles-->
                <div class="w-100 mw-250px">
                    <InputAjaxSelect :url="`/api/v1/admin/localisations/${$route.params.id}/titles`" name="title" :placeholder="$t('messages.select.title')" class="mb-0" v-model="title" :rules="{}" />
                </div>
                <!--end::Titles-->
        ...
        <div class="card-body pt-0">
            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5">
                ...
                <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                    ...
                    <th class="min-w-125px">{{ $t('attached') }}</th>
                ...
                <tr
                    ...
                    <!--begin::lastIssue=-->
                    <td>
                        <router-link v-if="model.last_issue" :to="{name: 'admin.issues.view', params:{id: model.last_issue.id}}" class="d-flex align-items-center">
                            <div class="d-flex justify-content-start flex-column">
                                <span class="text-dark fw-bolder text-hover-primary fs-6">{{ model.last_issue.name }} ({{ model.last_issue.date }})</span>
                            </div>
                        </router-link>
                    </td>
                    <!--end::lastIssue=-->
                    <!--begin::issues_count=-->
                    <td>
                        <router-link :to="{name: 'admin.versions.view', params:{id: model.id}}" class="text-dark text-hover-primary fs-6">{{ $tc('count.issues', model.issues_count) }}</router-link>
                    </td>
                    <!--end::issues_count=-->
                    <!--begin::Status=-->
                    <td>
                    ...
                    <td class="text-center">
                        ...
                        <label class="form-check form-switch form-switch-sm form-check-custom form-check-solid">
                            <input class="form-check-input" type="checkbox" v-model="model.is_attached" @input="handleToggle(model)"/>
                        </label>
                        <!--end::Switch-->
                    </td>
                    <!--end::Status=-->
                ...
                <tr class="odd">
                    <td colspan="9">{{ $t('no_version_found') }}</td>
                </tr>
                </tbody>
                <!--end::Table body-->
            </table>
...
export default {
    ...
    setup(){
        ...
        const loadModels = () => {
            ...
            ApiService.get('/localisations/' +  route.params.id + '/versions', {params: query}, true)
                .then(({data}) => {
                    if(data.success) {
        ...
        const handleToggle = (model) => {
            ApiService.put('/localisations/' +  route.params.id + '/versions/' + model.id)
                .then(response => {
                    if(response.data.success){
