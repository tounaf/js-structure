<template>
    ...
    <div v-if="model" class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <ToolBar>
            <!--begin::Buttons-->
            <router-link :to="{name: 'admin.localisations.edit'}" class="btn btn-sm btn-flex btn-primary fw-bolder me-2">{{ $t('edit') }}</router-link>
            <!--end::Buttons-->
        </ToolBar>
        <!--end::Toolbar-->
                ...
                <div class="card mb-6 mb-xl-9">
                    <div class="card-body pt-9 pb-0">
                        ...
                        <ul class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bolder">
                            <li class="nav-item">
                                <router-link :to="{name:'admin.localisations.view'}" :class="{'active': isActive('admin.localisations.view')}" class="nav-link text-active-primary py-5 me-6">{{ $t('versions') }}</router-link>
                            </li>
                        </ul>
                        <!--end::Nav-->
                    </div>
    ...
    setup(){
        ...
        onMounted(() => {
            store.dispatch(SET_BREADCRUMB, [
                ...
                {title: 'localisations', route: 'admin.localisations'},
                {title: 'Localisation'},
            ])
        })
        ...
        const loadModel = () => {
            ApiService.get('/localisations/' + route.params.id)
                .then(({data}) => {
                    if(data.success) {
