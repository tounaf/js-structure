<template>
    ...
    <div v-if="model" class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <ToolBar>
            <!--begin::Buttons-->
            <router-link :to="{name: 'admin.durations'}" class="btn btn-sm btn-flex btn-light fw-bolder me-2">{{ $t('back') }}</router-link>
...
export default {
    ...
    setup(){
        ...
        const create = () => {
            const data = getData()
            ApiService.post('/durations', data)
                .then((response) => {
                    if(response.data.success){
                        initModel()
                        toastr.success(t('messages.durations.created'))
                    }else{
        ...
        const update = () => {
            const data = getData()
            ApiService.put('/durations/' + id.value, data)
                .then((response) => {
                    if(response.data.success){
                        initModel(response.data.data)
                        toastr.success(t('messages.durations.updated'))
                    }else{
                        toastr.error(t('error'))
                    }
                })
        ...
        const loadModel = (value) => {
            if(!value){ return; }
            ApiService.get('/durations/' + value)
                .then((response) => {
                    if(response.data.success){
                        initModel(response.data.data)
                    }
