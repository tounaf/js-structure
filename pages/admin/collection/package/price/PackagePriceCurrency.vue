        ...
        <div class="card-body">
            ...
            <form @submit.prevent="submit" id="form" method="post" class="form fv-plugins-bootstrap5 fv-plugins-framework" novalidate="novalidate">
                ...
                <div
                    ...
                    :key="`price-item-${currency}-${item.duration_id}`"
                    class="row mb-6"
                >
    ...
    setup(){
        ...
        onMounted(() => {
            store.dispatch(SET_BREADCRUMB, [
                ...
                {title: 'versions', route: 'admin.packages'},
                {title: 'messages.prices.edit'},
            ])
        })
        ...
        const loadModels = () => {
            ...
            ApiService.get('/packages/' + id.value + '/currencies/' + currency.value + '/prices', null, true)
                .then(({data}) => {
                    if(data.success) {
        ...
        watch(() => currency.value, () => loadModels())
        ...
        const submit = () => {
            ...
            const data = getData()
            ApiService.put('/packages/' + id.value + '/currencies/' + currency.value + '/prices', data)
                .then((response) => {
